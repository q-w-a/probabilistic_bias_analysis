...
for (x in list(c(1e-3, 2e-3), c(1/3, 2/3))) {
  x.p <- f.beta(alpha, beta, x)        # The correct values of the p_i
  start <- (c(1e1, 1e1))            # A good guess is useful here
  sol <- nlm(objective, start, x=x, prob=x.p, lower=0, upper=1,
             typsize=c(1,1), fscale=1e-12, gradtol=1e-12)
  parms <- (sol$estimate)           # Estimates of alpha and beta
  #
  # Display the actual and estimated values.
  #
  print(rbind(Actual=c(alpha=alpha, beta=beta), Fit=parms))
...
for (x in list(c(.1200,.2188,.3683))) {
  # x.p <- f.beta(alpha, beta, x)        # The correct values of the p_i
  start <- (c(1e1, 1e1))            # A good guess is useful here
  sol <- nlm(objective, start, x=x, prob=c(.025,.5,.975), lower=0, upper=1,
             typsize=c(1,1), fscale=1e-12, gradtol=1e-13)
  parms <- (sol$estimate)           # Estimates of alpha and beta
  #
  # Display the actual and estimated values.
  #
...
rbeta(1e4, parms[1], parms[2]) %>% median()
rbeta(1e4, parms[1], parms[2]) %>% quantile(.025)
rbeta(1e4, parms[1], parms[2]) %>% quantile(.975)


rbeta(1e3, parms[1], parms[2]) %>% mean()
