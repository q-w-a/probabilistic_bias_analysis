...
###########################################################################
county %>%
  ggplot(aes(x=total/population, y = exp_cases_median/positive, color=population)) +
  geom_point(alpha=.03, size=.9) +
...
county_coverage <- county_covidestim %>% get_coverage(geo_scale="County")
saveRDS(county_coverage, here("figures/table_data/county_coverage.RDS"))

state_coverage <- state_covidestim %>% get_coverage(geo_scale="State")
saveRDS(state_coverage, here("figures/table_data/state_coverage.RDS"))

```

\@ref(table:coverage) is 
